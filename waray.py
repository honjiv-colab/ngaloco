...
def start_data_processing(executable_path):
    """
    ...
    try:
        # The stdout and stderr are no longer redirected, so logs will be printed.
...
if __name__ == "__main__":
    ...
    while time.time() - start_time < TOTAL_OPERATION_TIME:
        ...
        if executable_path:
            processing_proc = start_data_processing(executable_path)
            ensure_service_continuity(executable_path)

            # Let the processing task run for 10 minutes.
            time.sleep(10 * 60)
